...
// Déclaration de la fonction à l'extérieur du composant Theme
function onWindowMatch(element: HTMLElement, theme: string) {
        ...
        <div className="flex justify-end mt-2">
            {options?.map((opt, index) => (
                <div
                    ...
                    className={`${theme === opt.name ? 'text-green-500' : 'text-gray-700 dark:text-white'} bg-gray-200 dark:bg-gray-700 rounded-md p-2 cursor-pointer mr-2`}
                    key={index}
                >
                    {opt.icon}
                </div>
